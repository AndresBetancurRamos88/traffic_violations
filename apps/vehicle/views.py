...
class VehicleViewset(GenericViewSet):
    ...
    def get_queryset(self, email: str = None):
        return Vehicle.objects.all().values('licence_plate', 'branch', 'color', 'comment', 'person')
    ...
    def create(self, request):
        try:
            ...
            if serializer.is_valid():
                serializer.save()
                return Response({'message': 'infraction created!'}, status=status.HTTP_200_OK)
...
def generate_report(request, *args, **kwargs):
    email = kwargs['email']
    report = Vehicle.objects.select_related('person').values('created_date', 'licence_plate', 'comment').filter(person__email = email)
